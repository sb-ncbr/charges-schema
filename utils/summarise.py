...
(enumeration values are shown when --desc or “_full” diagram is requested)
"""
...
def format_range(m):
    if 'min' not in m and 'max' not in m: return ''
...
def parse_dictionary(doc, frame_names):
    ...
    cat_meta = defaultdict(dict)
    meta = defaultdict(dict)              # item_tag → {...}
    keys = defaultdict(OrderedDict)
    fks  = defaultdict(lambda: defaultdict(set))
    ...
    for fr in frames:
        ...
        if (cid := fr.find_value('_category.id')):
            cid = clean(cid)
            if (desc_raw := fr.find_value('_category.description')):
                cat_meta[cid]['desc'] = strip_cif_semicolons(desc_raw)
        ...
        for tag in fr.find_values('_item.name'):
            ...
            if (desc_raw := fr.find_value('_item_description.description')) and not m.get('desc'):
                m['desc'] = strip_cif_semicolons(desc_raw)

            # NEW ─ enumeration values
            enum_vals = fr.find_values('_item_enumeration.value')
            if enum_vals:
                m['enum'] = [clean(v) for v in enum_vals if clean(v)]

            # numeric ranges (unchanged)
            rows, rng_loop = [], fr.find('_item_range.', ['maximum', 'minimum'])
            if rng_loop:
                for row in rng_loop: rows.append((clean(row[1]), clean(row[0])))
            else:
                ...
                if mn or mx: rows.append((mn, mx))
            if rows:
                mins, maxs, ub_min, ub_max = [], [], False, False
                for mn, mx in rows:
                    nmn, nmx = num_or_none(mn), num_or_none(mx)
                    if nmn is None: ub_min = True
                    else: mins.append(nmn)
                    if nmx is None: ub_max = True
                    else: maxs.append(nmx)
                m['min'] = None if ub_min else (min(mins) if mins else None)
                m['max'] = None if ub_max else (max(maxs) if maxs else None)
                m['min_inc'] = any(num_or_none(a)==m['min']==num_or_none(b)
                                   for a,b in rows) if m['min'] is not None else False
                m['max_inc'] = any(num_or_none(a)==m['max']==num_or_none(b)
        ...
        if (child:=fr.find_value('_item_linked.child_name')) and (parent:=fr.find_value('_item_linked.parent_name')):
...
def generate_dot(cats, items, meta, keys, fks, cat_meta,
    ...
    for cat in cats:
        ...
        for tag in order:
            ...
            if tag in pk_set and tag in fk_set: name = f'<b><i>{name}</i></b>'
            elif tag in pk_set:                 name = f'<b>{name}</b>'
            elif tag in fk_set:                 name = f'<i>{name}</i>'
            ...
            cell = name
            if ann: cell += ' ' + ' '.join(f'[{escape_html(a)}]' for a in ann)
            rng = format_range(m)
            if rng: cell += ' ' + escape_html(rng)

            # NEW – enumeration
            if show_desc and m.get('enum'):
                cell += ' {' + ' | '.join(escape_html(v) for v in m['enum']) + '}'

            if show_desc and m.get('desc'):
                cell += '<br/><br/><font point-size="8">' + \
...
def print_text_report(cats, items, meta, keys, fks, show_desc=False):
    fk_lookup = {c:{ch for prs in g.values() for ch,_ in prs} for c,g in fks.items()}
...
def main():
    ...
    pa.add_argument('--desc', action='store_true',
                    help='add descriptions & enumeration values')
    args = pa.parse_args()
    ...
    if args.dot is not None:
        ...
        if args.dot == '-':
            print('----- Graphviz DOT -----'); print(dot)
        else:
            with open(args.dot, 'w', encoding='utf-8') as fh:
                fh.write(dot)
            print('DOT written to', args.dot)
